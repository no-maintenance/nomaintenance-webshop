...
import type {action as backInStockAction} from '~/routes.old/($locale).api.subscribe.restock';
import type {action} from '~/routes.old/($locale).api.subscribe.newsletter';
      ...
      <div className={'md:h-[40px] flex flex-col justify-end'}>
        {data?.errors &&
          Object.entries(data?.errors).map(([err, msg]) => (
            <p
              key={err}
              className={'text-destructive text-fine pb-1 flex items-center'}
            >
...
export function KlaviyoBackInStock({
  ...
  return (
    <fetcher.Form ref={formRef} method="post" action={`/api/subscribe/restock`}>
      ...
      <Button
        variant={'primary'}
        className={'bg-accent rounded py-2 mt-12 text-background uppercase'}
        width={'full'}
        type="submit"
        disabled={isLoading}
      >
